...
SELECT DISTINCT player FROM goal JOIN game ON id = matchid WHERE teamid <> 'GER' AND team1 = 'GER' OR team2 = 'GER' AND teamid != 'GER';
-- Show teamname and the total number of goals scored.
SELECT teamname, COUNT(teamname) AS '#goals' FROM eteam JOIN goal ON id=teamid GROUP BY teamname;
...
SELECT matchid, mdate, COUNT(matchid) AS 'goals' FROM goal JOIN game ON id = matchid WHERE teamid = 'GER' GROUP BY matchid, mdate;
